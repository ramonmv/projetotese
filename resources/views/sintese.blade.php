...
<head>
  <meta charset="UTF-8">
  <title>Hiperdidático - Síntese</title>
      ...
      <section class="flex-m flex-l nl3-m nr3-m nl3-l nr3-l">
        ...
        <article class="w-100 w-75-m w-75-l ph3-m ph3-l">
          <header class="mb3">
            ...
            <h2 class="ttu mt0 mb1 f6 fw5 blue">Síntese da sua compreensão</h2>
          ...
          @empty($sintese) 
            <form method="POST"  action="{{ route('addSintese', ['doc_id'=>$doc->id])  }}">
            <textarea  name="conteudo" id="conteudo" style="height:160px;color:#4f4f4f" placeholder="Elabore uma síntese de sua compreensão a respeito da leitura do texto, tendo como referência suas certezas e suas dúvidas."></textarea>     

          @else

            <form method="POST"  action="{{ route('editarSintese', ['doc_id'=>$doc->id, 'sintese_id'=>$sintese->id] )  }}">
            <textarea  name="conteudo" id="conteudo" style="height:160px;color:#4f4f4f">{!! $sintese->texto !!}</textarea> 
